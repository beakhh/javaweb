...
public class MemberLoginOkCommand implements MemberInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!pwd.equals(vo.getPwd())) {
			request.setAttribute("msg", "비밀번호를 다시입력해 주세요.");
			request.setAttribute("url", request.getContextPath()+"/MemberLogin.mem");
			return;
		}
	
		// 로그인 성공시에처리할 내용들을 기술한다.(1. 주요필드를 세션에 저장, 2. 오늘방문횟수처리, 3.총방문수와 방문포인트처리, 4.쿠키에 아이디 저장유무)
		// 1
		...
		// 오늘 처음 방문시는 오늘방문 카운트(todayCnt) 를 0으로 셋팅한다.
		if(!vo.getLastDate().substring(0,10).equals(strNow)) {
			dao.setTodayCntUpdate(mid);
			vo.setTodayCnt(0);
		}
		
		// 방문 포인트를 최대 50점 (5회 방문)까지 줄 수 있도록 처리....
		int nowTodayPoint = 0;
		if(vo.getTodayCnt() >=5) {
