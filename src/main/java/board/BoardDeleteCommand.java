...
public class BoardDeleteCommand implements BoardInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 정상적인 경로를 통해서 삭제하지 않을경우는 홈으로 돌려보낸다.
		...
		if(res == 1) {
			...
			request.setAttribute("msg", "게시글이 삭제실패~~");
			request.setAttribute("url", request.getContextPath()+"/BoardContent.bo?pag="+pag+"&pageSize="+pageSize+"&idx="+idx);
		}
