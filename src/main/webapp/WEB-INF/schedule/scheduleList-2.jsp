    ...
    <table class="table table-bordered" style="height:450px">
      ...
      <tr>
      	...
      	<c:forEach begin="1" end="${lastDay}" varStatus="st">
      	  ...
      	  <td id="td${gap}" ${todaySw==1 ? 'class=today' : ''} style="text-align:left; vertical-align:top">
      	 		${st.count}
      	 		
      	 		<!-- 해당 날짜에 일정이 있다면 part를 출력하게 한다. -->
      	 		<c:forEach var="vo" items="${vos}">
      	 			<c:if test="${fn:substring(vo.sDate,8,10)==st.count}"><br/>${vo.part}</c:if>
      	 		</c:forEach>
